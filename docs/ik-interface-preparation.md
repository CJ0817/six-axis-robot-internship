...
本页保留冻结目标的生成规则。2026-09-25状态更新：C解析IK已实现，运行结果与限制见[C解析逆解](c-analytic-ik.md)；下文“准备阶段”描述不代表当前求解状态。完整连杆推导、矩阵存储和现有ABI见[正运动学说明](forward-kinematics.md)；模型和坐标来源见[UR5模型约定](ur5-model-conventions.md)。
...
| 解析方法 | analytic_ur5；nearest_seed或all分支策略。q_seed用于选支，不能偷偷替换为目标见证q | 普通分支已实现；连续族选解有限支持 |
...
| C ABI | 候选输出容量、容量不足行为、结构体布局、版本及诊断字段 | 已冻结于abi-v1.md；robot_inverse_v1已实现普通解析分支 |
| 耗时/终止 | 单样本预算0.2 s；独立工作进程超时回收；计时层级及统计见统一规则 | 统一入口--suite ik已实现；运行结果另见C解析逆解报告 |

C ABI布局已冻结于[ABI v1](abi-v1.md)；解析几何分支、奇异族条件与等价角规则已整理于[UR5解析推导](ur5-analytic-ik.md)，附独立公式检查入口和140组回代证据。C逆解已实现；奇异族全局选解仍待实现。
...
固定目标、q_witness、q_seed、分组和最小奇异值均保存。准备报告的历史ik_execution_status=not_run保留；2026-09-25已运行C求解，新的成功率和分组结果见results/c-ik-verified/ik/report.json。误差有效数、RMSE、p95等继续遵循[统计规则](statistics-rules.md)。
