...
> 环境配置修订：已核对任务书第2～3页。正式技术基线为C11算法库、C++17编译支持、Python3.11.9/PyBullet3.2.7仿真、UR5 URDF；固定版本及验证状态见[环境配置](environment-setup.md)。下文第8节曾暂定的纯Python、可选物理仿真、型号待定仅保留为历史探索，已由本说明替代。积分仿真仍可作单元测试，但不能替代任务书PyBullet验证。任务书明确要求解析IK、三次/五次/梯形规划、直线/圆弧、避障、增量PID及速度/加速度前馈；此前数值IK/五次/P控制仅作为辅助基线，不构成对这些要求的删减。本次只完成环境配置；C算法与URDF/DH坐标映射仍需后续实现与验收。
