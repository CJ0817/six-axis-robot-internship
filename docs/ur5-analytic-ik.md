...
日期：2026-09-25。范围：与项目已核对 UR5 CB **标称几何**一致的解析推导；不是 UR5e，不含具体序列号的工厂几何标定，也不是实机验证。冻结 ABI 1.0.0 不变；C `robot_inverse_v1` 仍返回1008。本页的 Python 入口仅用于核验公式。
...
确认节点：实现 C IK 前审查解析模型匹配、奇异请求策略和域容差；C IK 合入前补齐独立 C FK回代、分支完整性、限位端点、全部错误码与超时专项。本次不改变 ABI 结构、签名或指标 KIN-03 的待实现状态。
