...
规范头文件为 `include/robot_kinematics.h` 与 `include/robot_errors.h`，机器可读快照为 `src/common/abi-v1.json`。既有 FK、矩阵函数和 robot_copy_joints 保持原签名与布局；ABI冻结时增加版本查询和版本化IK入口。2026-09-25已实现普通UR5解析IK，验证及奇异限制见[c-analytic-ik.md](c-analytic-ik.md)。
...
IK校验先检查必要指针及长度（1001；单独model为空为1004），再按有限性/模型/矩阵/选项约束、seed限位、算法请求顺序处理；DLS不收敛不能直接返回2001。失败结果不可执行，结果结构保持原字节；不通过同一个失败输出返回诊断数据，父进程另记耗时和返回码。

**2026-09-25：robot_inverse_v1已实现UR5解析分支、等价角筛选和FK回代。** KIN-03冻结140目标通过；DLS仍1008。精确腕奇异的all请求返回2003；nearest仅在seed已匹配目标时返回该零距离代表，其余连续族选解返回2003。布局、版本、签名与错误码编号不变。具体数值支持域与证据见[c-analytic-ik.md](c-analytic-ik.md)。
